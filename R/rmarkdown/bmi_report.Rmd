...
statement <- glue::glue_sql("SELECT * from bmi
                            WHERE Gender = {gender}", .con = bmi_database)
                             
bmi_dat <- DBI::dbGetQuery(conn = bmi_database,
                           statement = statement)
...
```{r bmi-EDA-predictive, eval = TRUE}
# 0. 환경설정 ------
library(caret)
library(doSNOW)
